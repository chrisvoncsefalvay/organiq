...
class _LabelSelectionTreeDelegate(ui.AbstractItemDelegate):
    ...
    def _build_group_widget(self, item: _LabelTreeItem, column_id: int) -> None:
        ...
        if column_id == 0:
            ...
            ui.Label(f"{item.group.title} ({selected}/{total})", style={"color": 0xFFE7E7E7})
    ...
    def _build_label_widget(self, item: _LabelTreeItem, column_id: int) -> None:
        ...
        if column_id == 0:
            ...
            ui.Label(f"{label.value}  {label.name}")
...
class OrganiqExtension(omni.ext.IExt):
    ...
    async def _dock_window(self):
        ...
        if viewport and window:
            window.dock_in(viewport, ui.DockPosition.LEFT, DOCK_RATIO)
    ...
    def _build_header(self):
        with ui.VStack(spacing=2, height=0):
            ...
            ui.Label("CT DICOM to USD anatomy with Isaac physics", height=18, style={"font_size": 12, "color": 0xFFB8B8B8})
            ...
            with ui.VStack(spacing=6, height=0):
                for status in dependency_status():
                    ...
                    with ui.HStack(height=22):
                        ...
                        ui.Label(status.dependency.required_for, word_wrap=True)
                ...
                if self._workflow.available_series:
                    for series in self._workflow.available_series:
                        with ui.HStack(height=36):
                            ...
                            ui.Label(summary, word_wrap=True)
    ...
    def _build_volume_preview_strip(self):
        if not self._volume_preview_providers:
            return
        with ui.HStack(height=118, spacing=6):
            for title, provider in self._volume_preview_providers:
                with ui.VStack(width=104, spacing=4):
                    ui.ImageWithProvider(provider, width=96, height=96)
                    ui.Label(title, alignment=ui.Alignment.CENTER, style={"color": 0xFFB8B8B8})
        ...
        with self._section_frame("select", "3. select objects to mesh"):
            with ui.VStack(spacing=5, height=0):
                ...
                with ui.HStack(height=28, spacing=6):
                    self._selection_summary_label = ui.Label(self._selection_summary_text(), width=126)
        ...
        with self._section_frame("export", "5. turn meshes into USD"):
            with ui.VStack(spacing=6, height=0):
                ...
                if self._workflow.state.usd_path is not None:
                    ui.Label(str(self._workflow.state.usd_path), word_wrap=True)
    ...
    def _build_instantiate_frame(self):
        with self._section_frame("instantiate", "6. instantiate with physics"):
            with ui.VStack(spacing=6, height=0):
                ...
                ui.Label(
                    "Bone labels become rigid bodies. Soft tissues become deformable volumes. The outer skin shell is a collision surface.",
                    word_wrap=True,
                )
    ...
    def _combo_row(self, label: str, options: tuple[str, ...], default: str):
        ...
        with ui.HStack(height=28, spacing=6):
            ui.Label(label, width=ROW_LABEL_WIDTH, style={"color": 0xFFC8C8C8})
            model = ui.ComboBox(index, *options, name="ComboBox", alignment=ui.Alignment.LEFT_CENTER).model
        return model
