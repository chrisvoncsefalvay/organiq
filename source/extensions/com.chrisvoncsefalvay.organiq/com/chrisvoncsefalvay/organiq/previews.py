...
def build_volume_preview_images(volume: Volume, size: int = 128) -> tuple[VolumePreviewImage, ...]:
    ...
    return (
        _preview_from_slice("axial", axial, size),
        _preview_from_slice("coronal", coronal, size),
        _preview_from_slice("sagittal", sagittal, size),
    )


def _preview_from_slice(name: str, slice_data, size: int) -> VolumePreviewImage:
    import numpy as np

    image = np.asarray(slice_data, dtype=np.float32)
    image = np.flipud(image)
    image = _window_ct_slice(image)
    image = _resize_nearest(image, size)
...
def _resize_nearest(image, size: int):
    ...
    target_height, target_width = _fit_size(width, height, size)
...
def _fit_size(width: int, height: int, size: int) -> tuple[int, int]:
    scale = float(size) / max(float(width), float(height), 1.0)
