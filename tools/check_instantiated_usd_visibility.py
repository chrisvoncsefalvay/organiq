...
async def _run() -> int:
    ...
    _require(capture_stats["red_tissue_pixels"] >= 100, "viewport capture has no visible tissue-coloured pixels")
...
async def _capture_viewport(capture_path: Path) -> dict[str, int]:
    ...
    stats = {"red_tissue_pixels": 0, "width": 0, "height": 0}
    try:
